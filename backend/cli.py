...
@click.option(
    ...
    type=click.INT,
    default=0,
    help="选填。限制最大采集数量，默认不限制",
)
@click.option(
    "-d", "--download", is_flag=True, help="选填。不需要下载时使用此参数，默认自动下载"
)
@click.option(
    "-t",
    "--type",
...
@click.option(
    ...
    type=click.STRING,
    default="下载",
    help="选填。下载文件夹，默认为[下载]",
)
@click.option(
    "-c",
    "--cookie",
    type=click.STRING,
    help="选填。已登录账号的cookie，可以直接填在config/cookie.txt文件中，也可在运行时手动输入，也可输入[edge/chrome]将会自动从本地浏览器读取cookie",
)
def main(urls, limit, download, type, path, cookie):
    if not urls:  # 未输入目标
        if type in ["like", "favorite", "follow", "fans"]:
            ...
            urls = (
                input(f"采集类型：{type}，请输入目标关键词/URL链接/ID或文件路径："),
            )
    for url in urls:
