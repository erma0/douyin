...
from .utils import quit, sanitize_filename, save_json, url_redirect
...
class Douyin(object):
        ...
        if self.target:  # 根据输入目标自动判断部分类型
            ...
            # 输入非链接
                ...
                # 用户uid
                    ...
                    "follow",
                    "fans",
                ] and id.startswith("MS4wLjABAAAA"):
    ...
    def __get_target_info(self):
        ...
        if self.type in ["search", "user", "live"]:
            ...
            pattern = (
                r'self\.__pace_f\.push\(\[1,"\d:\[\S+?({[\s\S]*?)\]\\n"\]\)</script>'
            )
            ...
            if self.limit == 0 or len(self.results) < self.limit:
                for item in awemes_list:
                    ...
                    if author:
                        ...
                        aweme["author_unique_id"] = author.get(
                    ...
                    if text_extra:
                        aweme["text_extra"] = [
                            {
                                "tag_id": hashtag.get(
                                    "hashtag_id", hashtag.get("hashtagId")
                                ),
                                "tag_name": hashtag.get(
                                    "hashtag_name", hashtag.get("hashtagName")
                                ),
                            }
