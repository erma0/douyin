...
@click.option('-mt', '--mstoken', default=False, is_flag=True, help='选填。指定是否在下载配置文件中设置UA及mstoken，默认不需要，出现下载0kb时尝试使用此参数')
...
@click.option('-b',  '--browser',
              type=click.Choice(["chrome", "msedge"], case_sensitive=False),
              default='msedge',
              help='选填。浏览器类型，默认使用EDGE，可选[chrome/msedge]，其他浏览器需修改browser文件')
@click.option('-p', '--path', type=click.STRING, default='下载', help='选填。下载文件夹，默认为[下载]')
@click.option('-pt', '--pathtype',
              type=click.Choice(["id", "title"], case_sensitive=False),
              default='id',
              help='选填。文件夹命名格式，默认为[type]_[id]，可选[title]（不需要作品增量采集时建议填title，即使用[标题/昵称/关键词]来命名）')
def main(urls, num, grab, download, login, mstoken, headless, type, browser, path, pathtype):
    ...
    if not urls:  # 未输入目标地址
        if type in ['like', 'favorite', 'follow', 'fans']:  # 采集本账号
            ...
            start(edge.context, urls, num, grab,
                  download, type, path, pathtype, mstoken)
            edge.stop()
            return
        elif type in ['post', 'music', 'search', 'collection', 'video']:  # 自动识别类型
            urls = (input('目标URL或文件路径：'), )
        else:
            logger.info('请输入目标URL或文件路径')
            return
    ...
    for url in urls:
        if os.path.exists(url):  # 文件路径
            ...
            if lines:
                for line in lines:
                    start(edge.context, line, num, grab,
                          download, type, path, pathtype, mstoken)
            else:
                logger.error(f'[{url}]中没有发现目标URL')
        else:
            start(edge.context, url, num, grab, download,
                  type, path, pathtype, mstoken)
    edge.stop()
