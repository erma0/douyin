...
@click.option('-h', '--headless', default=True, is_flag=True, help='选填。指定是否使用headless模式（默认为True，不显示浏览器界面），出现问题时使用此参数以便观察')
@click.option('-t',
              '--type',
...
def main(urls, num, grab, download, login, headless, type, browser, path, pathtype):
    """
    命令行
    """
    ...
    for url in urls:
        if os.path.exists(url):  # 文件路径
            ...
            if lines:
                for line in lines:
                    start(edge.context, line, num, grab, download, type, path, pathtype)
            else:
                logger.error(f'[{url}]中没有发现目标URL')
        else:
            start(edge.context, url, num, grab, download, type, path, pathtype)
    edge.stop()


def start(context, url, num, grab, download, type, path, pathtype):
    a = Douyin(context, url, num, type, path, pathtype)
