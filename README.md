...
v2.0 支持完整的服务器模式，Linux 用户无需安装 webview 依赖，直接用浏览器访问：
...
如需使用 PyWebView 桌面窗口，需要安装 webview 依赖。参考 [pywebview 官方文档](https://pywebview.flowrl.com/guide/installation.html#linux)。

Ubuntu 示例：

```bash
sudo apt install build-essential python3-dev libcairo2-dev libgirepository1.0-dev
```


## 服务器模式

后端基于 FastAPI 构建，提供完整的 RESTful API。
...
访问 `http://localhost:8000`（Docker 为 `http://localhost`）

```text
命令行参数:
    python -m backend.server              # 默认配置
...
v2.0 重构后采用标准的前后端分离架构：

- **后端**: FastAPI 提供 RESTful API + SSE 实时推送
- **前端**: React 通过 HTTP API 与后端通信
- **实时通信**: SSE (Server-Sent Events) 推送采集进度和结果

API 按功能模块划分：
- `/api/task/*` - 采集任务管理
- `/api/settings/*` - 应用设置
- `/api/aria2/*` - Aria2 下载服务
- `/api/file/*` - 文件操作
- `/api/system/*` - 系统工具
