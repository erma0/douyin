...
3. **收藏功能限制** - 仅支持获取自己账号的收藏
4. **自动获取Cookie无效** - 需要手动从浏览器复制Cookie
5. **搜索功能无效** - 关键词搜索功能可能无法正常工作
...
**首次配置：**
```powershell
.\scripts\setup\uv.ps1      # 配置 uv 环境
.\scripts\setup\aria2.ps1   # 下载 aria2
```

**开发运行：**
```powershell
.\scripts\dev.ps1           # 构建开发环境
...
```powershell
# PyInstaller 打包
.\scripts\build\pyinstaller.ps1 -Mode dir      # 目录模式（推荐）
.\scripts\build\pyinstaller.ps1 -Mode onefile  # 单文件模式

# Nuitka 打包
.\scripts\build\nuitka.ps1 -Mode dir           # 目录模式（性能好）
.\scripts\build\nuitka.ps1 -Mode onefile       # 单文件模式（慢）
...
#### 选择建议
- 日常使用：PyInstaller 目录模式
- 分发给他人：PyInstaller 单文件模式
- 追求性能：Nuitka 目录模式
